...
function PickupOrders(props) {
  ...
  let m = props.orders.filter(x => dayjs(x.date + " " + x.time).isBefore(dayjs()) && x.state !== "pending" && x.pickup === 1).map(s => s.order_id).filter(onlyUnique);
  ...
  return (<>
    ...
    <table className="mx-3 text-center table table-striped table-hover table-responsive">
      ...
      <tbody>
        {props.orders.filter(x => dayjs(x.date + " " + x.time).isBefore(dayjs()) && x.state !== "pending" && x.pickup === 1).map((s) => {
