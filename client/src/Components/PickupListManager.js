...
function PickupListManager(props) {
  ...
  useEffect(() => {
    ...
    condensedOrders.forEach((order_obj) => {
      ...
      order_array.forEach((order) => {
        if (order.pickup === 0 || order.state === 'pending' || order.state === 'shipped' || order.state === 'delivered') {
          deleteOrderFlag = true;
        }
        if (order.farmer_state === 'farmer-shipped') {
          order_obj.farmer_shipped = true;
        }
        else if (order.farmer_state === 'received') {
          order_obj.warehouse_received = true;
        }
        else if (order.farmer_state === 'prepared') {
  ...
  const CannotPreparePopover = (farmer_shipped, warehouse_received, order_not_shipped) => (
    <Popover id="popover-basic">
      <Popover.Header as="h3">Order not ready for pick-up</Popover.Header>
      <Popover.Body className="text-wrap">
        {farmer_shipped && <span>Order has been shipped by the farmer but has not yet been received by the warehouse</span>}
        {warehouse_received && <span>Order has been received by the warehouse but has not yet been prepared</span>}
        {order_not_shipped && <span>Order has not yet been shipped by the farmer</span>}
      </Popover.Body>
    </Popover>
  );
          ...
          <tbody>
            {orders.map((s) => (
              <tr key={s.id}>
                ...
                <td>
                  {!s.order_prepared &&
                    <OverlayTrigger trigger="hover" overlay={() => (CannotPreparePopover(s.farmer_shipped, s.warehouse_received, s.order_not_shipped))}>
                      <span>{infoIcon}</span>
                    </OverlayTrigger>
                  }
                </td>
