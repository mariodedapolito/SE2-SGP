import { Button, Row, Col, Form, Alert, Modal, Table, Badge, Container } from 'react-bootstrap';
...
function DeliverList(props) {
  ...
  useEffect(() => {
    ...
    const handoutOrder = async () => {
      try {
        let array2 = props.orders.filter(x => x.order_id === handoutOrderID).map(x => x.product_name);
        for (const a of array2) {
          await API.updateDelivered(handoutOrderID, a);
        }
        setActionAlert({ variant: 'success', msg: 'Order #' + handoutOrderID + ' was successfully handed out.' });
      }
      catch (error) {
        console.log(error);
        setActionAlert({ variant: 'danger', msg: 'Order #' + handoutOrderID + ' could not be handed out.' });
      }
  ...
  const getOrderStatus = (order_id) => {
    ...
    props.orders.filter((o) => (o.order_id === order_id)).forEach((item) => {
  ...
  return (<>
    ...
    <h5 className="d-block mx-auto mb-5 text-center text-muted">
      View and manage all the client orders by using the buttons on the table below
    </h5>
    <Container fluid>
      <Row>
        {actionAlert &&
              ...
              if (!m.find(x => (parseInt(x) === parseInt(s.order_id)))) {
                ...
                return (
                  <tr key={s.id}>
                    ...
                    <td className='align-middle'>{props.clients.find((c) => (c.client_id === s.client_id)).name} {props.clients.find((c) => (c.client_id === s.client_id)).surname} </td>
                    ...
                    {(!dayjs(time.date + " " + time.hour).isSameOrAfter(s.date + " " + s.time) || s.pickup === 0) && <td className='align-middle'>{dayjs(s.date + ' ' + s.time).format("ddd, MMM D, YYYY HH:mm")}</td>}
                    <td>
                      {s.state === 'prepared' && s.pickup === 1 &&
                        <Button variant="success" className="d-block my-1 mx-2 w-100" onClick={() => {
                      ...
                      {s.state === "pending" &&
                        <Button variant="secondary" className="d-block my-1 mx-2 w-100" onClick={() => {
                          setShowClient(true);
                      ...
                      {dayjs(time.date + " " + time.hour).isSameOrAfter(s.date + " " + s.time) && s.pickup === 1 &&
                        <Button variant="secondary" className="d-block my-1 mx-2 w-100" onClick={() => {
                          setShowContact(true);
                          setClient(s.client_id)
                          setContactType(1)
                      ...
                      {!dayjs(time.date + " " + time.hour).isSameOrAfter(s.date + " " + s.time) || s.pickup === 0 &&
                        <Button variant="primary" className="d-block my-1 mx-2 w-100" onClick={() => {
                          setShowContact(true);
                          setClient(s.client_id)
                          setContactType(0)
                      ...
                      {s.pickup === 1
                        && dayjs(time.date + " " + time.hour).isSameOrBefore(s.date + " " + s.time)
                        && dayjs(s.date + " " + s.time).diff(time.date + " " + time.hour, 'hour') < 24
                        &&
                        <Button variant="dark" className="d-block my-1 mx-2 w-100" onClick={() => {
                          setShowContact(true);
                          setClient(s.client_id)
                          setContactType(0)
                          if (Math.ceil(Math.abs((new Date(s.date) - new Date(time.date))) / (1000 * 60 * 60 * 24)) <= 1) {
    ...
    <Finestra show={show} setShow={setShow} handleClose={handleClose} id={id} orders={props.orders} products={props.products} />
    <OrderStatus show={showOrderStatus} setShow={setShowOrderStatus} orders={props.orders} id={orderStatusID} />
      ...
      <Modal size="md" show={props.show} onHide={props.handleClose} animation={false}>
        ...
        {props.clients.filter(x => x.client_id === props.client).map((s) =>
          <Modal.Body key={s.id}>
            ...
            <span className='d-block text-center text-danger mb-3'> This order is pending due to the insufficient balance. Select the way for contacting the Client </span>
            <hr />
            <Row className="mb-3">
              <Col sm={8}><h5><Telephone size={32} /> {` ${s.phone}`}</h5></Col>
              <Col sm={4}><div className="d-block text-end"><Button variant="primary">Phone call</Button></div></Col>

            </Row>
            <Row>
              <Col sm={8}><h5><Envelope size={32} />{` ${s.email}`}</h5></Col>
              <Col sm={4}>
                <div className="d-block text-end">
                  <Button variant="primary" disabled={emailSent} onClick={() => {
                    changeEmailState(s)
                    handleOpenDialog()
                  }}
                  >
    ...
    <Modal size="md" show={props.show} onHide={props.handleClose} animation={false}>
      ...
      {props.clients.filter(x => x.client_id === props.client).map((s) =>
        <Modal.Body key={s.id}>
          ...
          <span className='d-block text-center text-danger mb-3'> This order has not been picked up in time by the client. Send an email to the client to remind the missed pick-up. </span>
          <hr />
          <Row>
            <Form.Group>
              <div className='d-block text-center mb-2 lead'>Write an email to {s.email}:</div>
            ...
            <div className='d-block text-center'>
              {contactType === 0 && notify == 0 && <Button variant="secondary" className='mt-2'
                onClick={() => {
                  setMailerState((prevState) => ({
                    ...prevState,
                    email: s.email,
              ...
              {contactType === 1 && notify === 0 && <Button variant="secondary" className='mt-2'
                onClick={() => {
                  setMailerState((prevState) => ({
                    ...prevState,
                    email: s.email,
              ...
              {notify === 1 &&
                ...
                > Apply pick-up in the next 24h template</Button>
              }
            </div>
          </Row>
